...
// Local type shims for internal DO operations to satisfy TS
...
export interface Env {
  GlobalDurableObject: {
    ...
    get(id: any): DurableObjectStub<GlobalDurableObject>;
  };
}
export interface DurableObjectStub<T> {
  getDoc<T>(key: string): Promise<Doc<T> | null>;
  ...
  indexAddBatch(items: any[]): Promise<void>;
  indexRemoveBatch(items: any[]): Promise<number>;
  indexDrop(rootKey: string): Promise<void>;
}
...
export class GlobalDurableObject extends DurableObject<Env> {
  ...
  async listPrefix(prefix: string, startAfter?: string | null, limit?: number) {
    ...
    const m = await this.ctx.storage.list<unknown>(opts);
...
export abstract class Entity<State> {
  ...
  constructor(env: Env, id: string) {
    ...
    this.stub = env.GlobalDurableObject.get(doId) as any;
  }
...
export class Index<T extends string> extends Entity<unknown> {
  ...
  async page(cursor?: string | null, limit?: number): Promise<{ items: T[]; next: string | null }> {
    ...
    return { items: keys.map((k: string) => k.slice(2) as T), next };
  }
  async list(): Promise<T[]> {
    const { keys } = await this.stub.listPrefix('i:');
    return keys.map((k: string) => k.slice(2) as T);
  }
}
