...
import { ChevronLeft, Loader2, Calendar, User, ArrowRight, ExternalLink, Share2, Clock, ShieldAlert } from 'lucide-react';
  ...
  if (isPostLoading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-background gap-6">
        ...
        <p className="text-sm font-bold uppercase tracking-[0.2em] text-muted-foreground">Establishing Node Connection...</p>
      </div>
    );
  }
  // Fallback content if Hive post doesn't exist but we have a record (demo mode)
  const displayPost = post || (record ? {
    author: record.metadata.author,
    permlink: record.metadata.permlink,
    title: record.metadata.title || "Simulated Content",
    body: `<p>${record.metadata.intro || ""}</p><p>${record.metadata.description || ""}</p><p><em>Note: This is a simulated transmission from the Hive Demo Layer. In a live environment, the full content would be retrieved from the Hive blockchain.</em></p>`,
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-background p-4 text-center space-y-8">
        ...
        <div className="space-y-4">
          ...
          <p className="text-muted-foreground max-w-md mx-auto">This niche broadcast is no longer available. It may have been archived or moved.</p>
        </div>
        <Button asChild variant="outline" className="rounded-full px-8 h-12 font-bold shadow-lg">
  ...
  return (
    <div className="min-h-screen bg-background relative selection:bg-amber-500/30 overflow-x-hidden">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-16">
        ...
        {isDemo && (
          <div className="mb-8 p-4 bg-blue-500/10 border border-blue-500/20 rounded-2xl flex items-center gap-3 text-blue-600 dark:text-blue-400">
            <ShieldAlert className="w-5 h-5 shrink-0" />
            <p className="text-sm font-bold uppercase tracking-wider">Simulated Transmission: Viewing Demo Content</p>
          </div>
        )}
        <article className="space-y-10">
          <header className="space-y-8">
            <div className="flex flex-wrap items-center gap-4">
              ...
              <div className="flex items-center gap-5 text-[11px] font-bold uppercase tracking-widest text-muted-foreground/80">
                <span className="flex items-center gap-2"><Calendar className="w-3.5 h-3.5 text-amber-500/60" /> {format(new Date(displayPost.updated || displayPost.created), 'MMMM d, yyyy')}</span>
                <span className="flex items-center gap-2"><User className="w-3.5 h-3.5 text-amber-500/60" /> @{displayPost.author}</span>
            ...
            {coverImage && (
              <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="overflow-hidden rounded-3xl shadow-2xl border border-white/10">
          ...
          <div className="prose">
            <div className="hive-content-body" dangerouslySetInnerHTML={{ __html: displayPost.body }} />
          </div>
          <footer className="mt-20 pt-16 border-t border-border space-y-20">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              <section className="space-y-6">
                ...
                <div className="p-8 rounded-3xl bg-secondary/30 border border-border/50 backdrop-blur-sm text-sm text-muted-foreground">
                  This content is curated into the Niche Hive for its unique technical perspective and linkable insights.
                </div>
              </section>
              <section className="space-y-6">
                <h3 className="text-2xl font-black tracking-tight">Nodes & Mirrors</h3>
